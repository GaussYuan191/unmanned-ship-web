<template>
  <div>
    ...
    <el-card>
      ...
      <el-table :data="shipInfoList" border stripe v-loading="getloading">
        ...
        <el-table-column label="操作" fixed="right" width="165">
          <template slot-scope="scope">
            <!-- 下拉菜单 -->
            <el-dropdown @command="handleCommand" placement="bottom-start" @visible-change="getShipInfoById(scope)">
        ...
        <el-row v-if="viewRunStatusFrom.state != '离线'">
          ...
          <el-col :span="11">
            <el-form-item label="俯仰角">
              <el-input v-if="!viewRunStatusFrom.pitch || viewRunStatusFrom.state == '离线'" :value="``" readonly></el-input>
              <el-input v-else v-model="Number(viewRunStatusFrom.pitch).toFixed(2) + `°`" readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="电池电量">
              <el-input v-if="!viewRunStatusFrom.battery || viewRunStatusFrom.state == '离线'" :value="``" readonly></el-input>
              <el-input v-else v-model="Number(viewRunStatusFrom.battery * 100).toFixed(2) + '%'" readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="翻滚角">
              <el-input v-if="!viewRunStatusFrom.roll || viewRunStatusFrom.state == '离线'" :value="``" readonly></el-input>
              <el-input v-else v-model="Number(viewRunStatusFrom.roll).toFixed(2) + `°`" readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="方位角">
              <el-input v-if="!viewRunStatusFrom.yaw || viewRunStatusFrom.state == '离线'" :value="``" readonly></el-input>
              <el-input v-else v-model="Number(viewRunStatusFrom.yaw).toFixed(2) + '°'" readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="航行速度">
              <el-input v-if="!viewRunStatusFrom.velocity || viewRunStatusFrom.state == '离线'" :value="``" readonly></el-input>
              <el-input v-else v-model="Number(viewRunStatusFrom.velocity).toFixed(2) + 'm/s'" readonly></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <!-- 执行计划 -->
    <el-dialog title="执行计划" :visible.sync="actionPlanDialogTableVisible" width="30%" center @close="actionClose">
    ...
    >
      <div class="return-home-box">
        ...
        ></Amap>
        <el-button type="primary" @click="openEdit" v-show="!isClick" class="return-home-edit">编辑</el-button>
        <div class="return-home-edit">
          <el-button type="primary" @click="updataEdit" v-show="isClick" :loading="upDataLoading">保存</el-button>
    ...
    <!-- 实时视频dialog -->
    <el-dialog title="实时视频" :visible.sync="onlineVideoDialogVisible" width="650px" center @close="closeOnlineVideo" destroy-on-close>
...
export default {
  ...
  methods: {
    ...
    async showConfigInfo(scope) {
      ...
      //   this.masterConfigFrom.thresholdBatteryLevel = Math.round(this.masterConfigFrom.thresholdBatteryLevel * 100);
      //   // 深克隆
    ...
    btneditConfig() {
      this.$refs.configFormRef.validate(async (val) => {
        ...
        if (val || this.configFrom.enableLowBatteryReturn == false) {
          ...
          const { data: res } = await this.$http.post(`/usv/setconfig?id=${this.shipId}`, this.masterConfigFrom);
          if (!res.error_code) {
    ...
    async viewOnlineVideo(scope) {
      ...
      // }
      this.iframeUrl = 'http://124.222.218.232:8000/live/STREAM_NAME.flv';
    },
    //获取实时视频token函数
    async getOnlineVideoToken() {
      const { data: videoRes } = await this.$http.get(`/camera/accesstoken`);
      if (!videores.error_code) {
        // 保存token,和过期时间
  ...
  computed: {
    ...
    setLocation() {
      return Number(this.viewRunStatusFrom.location.latitude) + ',' + Number(this.viewRunStatusFrom.location.longitude);
    },
  },
  beforeMount() {},
  mounted() {
